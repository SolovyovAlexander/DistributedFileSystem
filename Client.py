...
def instruction_resolver(instruction):
    if instruction == 'init':
        ...
        inst, filename = instruction.split(' ')
        return ClientRequest.FileCreate(request_type=inst, filename=filename)
...
class Client(Thread):
    def run(self) -> None:
        ...
        while True:
            instruction = input()
            ns_connection = ClientRequest.MessageProvider('localhost', 9001)
            if instruction != 'exit':
                client_req = instruction_resolver(instruction)
                client_req.execute(ns_connection)
            else:

                ns_connection.sock.close()
                break
